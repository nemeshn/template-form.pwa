/*
  Author: lgapontes
  Respository: https://github.com/lgapontes/behappywith.me
  local: behappywith.me/front-end/src/components/ImageScroller/ManipularEvento.js
*/
...
class ManipularEvento {
  constructor(qtdeElementos, index) {
    ...
    this.minLeft = (
      (qtdeElementos - 1) * this.comprimentoItem * (-1)
    ) + this.maxLeft;
  ...
  atualizar(toque) {
    ...
    if (toque) {
      this.corrigirIndex();
      this.left = (
        this.index * this.comprimentoItem * (-1)
      ) + this.maxLeft;
    }
  }
  ...
  flinging(toqueX) {
    ...
    const diferenca = toqueX - this.toqueAnterior;
    const extensaoToque = Math.abs(diferenca);
